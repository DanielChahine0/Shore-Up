...
export function Logo({ compact = false }: { compact?: boolean }) {
  return (
    <Link href="/" className={`glass flex h-11 shrink-0 items-center gap-2.5 rounded-full pl-1.5 ${compact ? "pr-1.5 sm:pr-4" : "pr-4"}`} aria-label="Shore Up home">
      <span className="flex h-8 w-8 items-center justify-center rounded-full bg-brand text-white">
        <LogoMark className="w-5" />
      </span>
      {/* On narrow page headers the mark stands alone so the CN pill keeps its room. */}
      <span className={`text-[15px] font-bold uppercase tracking-wide text-ink ${compact ? "hidden sm:inline" : ""}`}>Shore Up</span>
    </Link>
  );
}
