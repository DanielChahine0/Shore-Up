...
const primary = "h-11 w-full rounded-full bg-foam text-sm font-semibold text-foam-deep disabled:opacity-60";
const quiet = "h-11 rounded-full border border-line px-4 text-sm font-medium text-mist hover:text-shell";
...
export function TrashSheet(props: TrashSheetProps) {
  ...
  return (
    <>
      ...
      */}
      <button type="button" aria-hidden tabIndex={-1} onClick={onClose} className="fixed inset-0 z-20 cursor-default bg-abyss/50 sm:hidden" />
      ...
      >
        <header className="flex shrink-0 items-start justify-between gap-3 px-5 pb-1 pt-5">
          <div className="min-w-0">
            <h2 ref={headingRef} tabIndex={-1} className="rounded text-lg font-semibold tracking-tight text-shell outline-offset-2">
              {justFinished ? "Session finished" : session ? "Clean session" : "Start a clean session"}
            </h2>
            {beach && !justFinished && <p className="mt-1 truncate text-sm text-mist">at {beach.name}</p>}
          </div>
          <button type="button" onClick={onClose} aria-label="Close clean session" className="-mr-1.5 shrink-0 rounded-full p-1.5 text-mist hover:text-shell">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 pb-5 pt-2">
          ...
          {justFinished ? (
            ...
            <>
              ...
              {error && (
                <p role="alert" className="mt-4 rounded-xl border border-line bg-navy px-3 py-2 text-sm text-shell">
                  {error}
                </p>
              )}
              ...
              <div className="mt-5">
                ...
                {!canFinish && <p className="mt-1.5 text-center text-xs text-mist">Log at least one item to finish.</p>}
              </div>

              <div className="mt-3 flex items-center justify-center gap-2">
                {confirmDiscard ? (
                  <>
                    <p className="text-sm text-mist">Throw this session away?</p>
...
function UnlockProgress({ progress }: { progress: ReturnType<typeof unlockProgress> }) {
  if (progress.unlocked) {
    return (
      <p className="fade-in mt-2 inline-flex items-center gap-1.5 rounded-full bg-foam px-3 py-1 text-xs font-semibold text-foam-deep">
  ...
  return (
    <div className="mt-2">
      ...
      <div
        ...
        className="mt-1.5 h-1.5 w-full max-w-40 overflow-hidden rounded-full bg-tide"
      >
        <div className="h-full rounded-full bg-foam transition-[width] duration-300 ease-out" style={{ width: `${(progress.total / progress.needed) * 100}%` }} />
      </div>
    </div>
  );
}
...
function StartScreen({ beach, signedIn, pending, saving, error, onStart, onSavePending }: StartProps) {
  return (
    <>
      <p className="text-sm leading-relaxed text-shell">
      ...
      <p className="mt-2 text-sm text-mist">Five items in one session unlocks badges and levels.</p>

      {pending.length > 0 && (
        <div className="mt-4 rounded-2xl border border-line bg-navy p-3">
          ...
          <ul className="mt-2 space-y-2">
            {pending.map((entry) => (
              <li key={entry.startedAt} className="flex items-center justify-between gap-3">
                ...
                {signedIn ? (
                  <button type="button" onClick={() => onSavePending(entry)} disabled={saving} className="h-11 rounded-full border border-foam/60 px-4 text-sm font-semibold text-foam disabled:opacity-60">
                    {saving ? "Saving" : "Save it"}
                  </button>
                ) : (
                  <Link href="/signin?next=/" className="flex h-11 items-center rounded-full border border-foam/60 px-4 text-sm font-semibold text-foam">
                    Sign in to save
                  </Link>
                )}
              </li>
      ...
      {!signedIn && <p className="mt-2 text-center text-xs text-mist">You can run a session as a guest. Signing in afterwards saves it.</p>}
    </>
  );
}
  ...
  return (
    <>
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0">
          <p className="text-2xl font-semibold tracking-tight text-shell">{itemLabel(total)}</p>
          <p className="mt-1 text-sm text-mist">collected{session.beachName ? ` at ${session.beachName}` : ""}</p>
        </div>
      </div>
      <p className="mt-4 text-sm leading-relaxed text-shell">
