...
const step = "flex h-11 w-11 items-center justify-center rounded-full border border-line text-shell transition-colors hover:bg-tide disabled:border-line/60 disabled:text-mist/50 disabled:hover:bg-transparent";
    ...
    <ul className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
      {TRASH_ITEMS.map((item) => {
        ...
        return (
          ...
          <li key={item.key} role="group" aria-label={`${item.name}: ${count}`} className="flex h-full flex-col rounded-2xl border border-line bg-navy/60 p-2.5">
            <div className="flex flex-col items-center gap-1.5">
              <span className="relative">
                <Icon className={`h-7 w-7 ${count > 0 ? "text-foam" : "text-mist"}`} />
                {count > 0 && (
                  <span aria-hidden className="absolute -right-2.5 -top-2 min-w-[1.25rem] rounded-full bg-foam px-1 text-center text-xs font-semibold leading-5 text-foam-deep">
                    {count}
                  </span>
                )}
              </span>
              <span aria-hidden className="text-center text-xs leading-tight text-mist">
                {item.name}
              </span>
            </div>
